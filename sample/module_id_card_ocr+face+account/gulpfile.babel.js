...
const clean_task = () =>
  gulp.src(BUILD_DIR, { allowEmpty: true }).pipe(clean({ force: true }));
...
const css = (src, dest) =>
  gulp.src(src).pipe(importCss()).pipe(gulp.dest(dest));

const html = (src, dest) => gulp.src(src).pipe(gulp.dest(dest));
...
export const build = gulp.series(
  clean_task,
  gulp.parallel(demo_js_build, demo_css_build, demo_html_build, demo_res_build)
);

export const debug = gulp.series(
  clean_task,
  gulp.parallel(demo_js_debug, demo_css_build, demo_html_build, demo_res_build)
);
